...
const RegularExpenseService = (() => {
    ...
    const deleteRegularExpense = async (input) => {
        return new Promise(((resolve, reject) => {
            try {
                RegExpense.findOneAndRemove({_id: input.id, userId: input.userId}, (err, doc, res) => {
                    if (err) reject({error: true, message: err.message})
                    else resolve(doc);
                });
            } catch (err) {
    ...
    const updateRegularExpense = async (input) => {
        return new Promise((resolve, reject) => {
            try {
                RegExpense.findOneAndUpdate({_id: input.id, userId: input.userId}, input, {new: true}, (err, res) => {
                    if (err) reject({error: true, message: err.message})
                    else resolve(res);
                })
            } catch (err) {
